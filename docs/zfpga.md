...
| `zfpga reboot` | assert PROGRAMN | -- | 7, step 1 |
| `zfpga boot blink.bit` | write the gateware slot and verify | `openFPGALoader -f` | 7 |
...
- **Output** reaches the terminal through the `stdout` relay, which the
  ...
  can jump to them.
- **Exit status** is real, so `zfpga build blink.v && zfpga boot blink.bit`
  works as it reads.
- **No prompting.** The relay is one-way and input typed while a child
  runs is dropped. So `zfpga boot` cannot ask "are you sure?" -- and
  ...
  **writing is separated from leaving**: `zfpga boot` writes and verifies,
  `zfpga reboot` is a second, deliberate command. A write can be checked
...
| **`zfpga boot` writes; `zfpga reboot` leaves** | `posix` cannot prompt, and a verified write should be separable from giving up the OS (§4.4). |
