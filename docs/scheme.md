...
`sw/apps/lisp` embeds [Machdyne Scheme](https://github.com/machdyne/ms)
...
1. `ms_to_string()` made non-`static` (a plain visibility change, no
   ...
   a port or into a `Z_LISP_RESULT` reply instead of this process's
...
actual size turned out to be. For `lisp`'s own small static footprint,
...
`lisp`'s own `.bss`, which means it's now counted as part of the
...
`sw/apps/lisp/Makefile`'s `MS_HEAP_SIZE`/`MS_PROTECT_STACK_SIZE`
...
- **One shared heap for the whole `lisp` process**, not one per
  connection -- see `lisp.c`'s own header comment on why (one shared
  `ms_global_env`, run a second `lisp` instance for real isolation).
...
`sw/apps/lisp/gen_ms_stdlib.py` -- see that script's own header
...
whatever's actually in the stdlib) is non-fatal to `lisp` itself --
...
`sw/apps/lisp/lisp.c`. Builtins always win on a match first (typing
...
across more than one line isn't handled), and `Z_LISP_EVAL` requests
(the non-interactive path, see "LISP_EVAL" below) only evaluate the
...
exact usage pattern this requires, and `sw/apps/lisp/ms_api.h` for why
it can't be wrapped in a shared helper) -- a bad form reports an error
back down the port/reply message, it doesn't take the whole `lisp`
...
`ms_to_string()`, the function `lisp.c` actually calls) detects a
...
present, just in the wrong half. Confirmed on-target via `lisp`'s own
...
in `ms_api.h`. `free` (`sw/apps/lisp/lisp.c`) reports three numbers,
none of which needed a new syscall:

- **Scheme heap** -- cells used/total (`ms_heap_used()`/`MS_HEAP_SIZE`,
  the latter already known to `lisp.c` at compile time via the same
...
- **C heap** -- bytes grown via `malloc()` since boot, computed as
  ...
  idiom) -- for `lisp` specifically this is almost entirely `ms`'s own
...
instance that typed it to disconnect from `lisp` and connect to a
...
`term`'s own pid (`lisp` already knows it, it's the connection's own
...
(`connect_port("lisp0", Z_PID_LISP)`, with the well-known default's
fixed-pid fallback) and on `Z_TERM_SET_PORT` (`connect_port(name, 0)`
-- no fallback for a caller-specified name, if the lookup fails
there's nothing sensible to guess). `lisp` closes its own end of the
...
text `lisp` sends back very likely never actually appears on screen --
