...
/*
 * A syscall trampoline for host builds of an app's tests.
 *
 * -- why this is needed --
 *
 ...
 *
 * This lived in sw/apps/hex/tests until sw/apps/sheet's render wanted
 * it too, which is the condition its own note said to promote it on.
 * It is still a SEPARATE header from zrender.h rather than folded into
 * it: a render that never sends a message does not want this, and an
 * unused static function is a -Wunused-function warning in every one
 * of them. sw/apps/hex/tests/trampoline.h is now a one-line shim onto
 * this file, so nothing that included it had to change.
 *
