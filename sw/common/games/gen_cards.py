#!/usr/bin/env python3
#
# Zeitlos -- card tile generator.
#
#   cd sw/common/games && python3 gen_cards.py
#
# Emits zcardart.c / zcardart.h. Generated C is committed alongside the
# generator, and `make cards` in any of the card apps runs it here.
#
# THE OUTPUT IS COMMITTED, SO IT MUST REGENERATE IDENTICALLY. When this
# moved out of sw/apps/poker the file names were renamed but the strings
# written INTO them were not, so re-running it produced a header with
# the old include guard and a reference to a file that no longer exists.
# Nothing would have failed to compile; the tree would just have drifted
# away from its own generator. Check with:
#
#     cd sw/common/games && python3 gen_cards.py && git diff --stat
#
# Generated C is committed the same arrangement sw/apps/chess/gen_pieces.py,
...
SUITS = ['club', 'diamond', 'heart', 'spade']       # pk_zcardart.h order
SUIT_CH = ['c', 'd', 'h', 's']
RED = {'heart', 'diamond'}
...
def mini_card(rank, suit):
    ...
    That is not decoration. Overlapped at Z_ART_MINI_STRIDE the only
...
#define Z_ART_FULL_W %d
#define Z_ART_FULL_H %d
#define Z_ART_MINI_W %d
#define Z_ART_MINI_H %d
...
""" % (CARD_W, CARD_H, MINI_W, MINI_H))
    ...
    with open("zcardart.c", "w") as f:
        f.write("/* Generated by gen_cards.py -- do not hand-edit. */\n\n")
        f.write('#include "zcardart.h"\n\n')
        emit_array(f, "z_card_full", full, CARD_H)
        emit_array(f, "z_card_mini", mini, MINI_H)
        f.write("const uint32_t z_back_full[] = {\n")
        for w in rows_to_words(back(CARD_W, CARD_H)):
            f.write("    0x%08x,\n" % w)
        f.write("};\n\nconst uint32_t z_back_mini[] = {\n")
        for w in rows_to_words(back(MINI_W, MINI_H)):
            f.write("    0x%08x,\n" % w)
        f.write("};\n")

    print("wrote zcardart.c and zcardart.h")
