...
static int led_prompt_inner(sbuf *sb, char *insert, int *kmap, ins_state *is, int ps, int flg)
{
	int n, key, off;
...
static int led_input_inner(sbuf *sb, char *post, int postn, int row, int flg, int *pren)
{
	int ai_max = 128 * xai;
...
/*
 * ZEITLOS: the two functions that read TYPED text, wrapped so that the
 * host knows when the editor is expecting characters rather than
 * commands.
 *
 * sw/apps/vi/vi_zeitlos.c translates the terminal's arrow-key escape
 * sequences into h/j/k/l, which is the only way to make arrows move
 * the cursor in an editor that binds no escape sequences (real vi does
 * not either). That translation must NOT happen while text is being
 * typed, or an arrow key inserts the letter into the document.
 *
 * Wrapped rather than incremented in place because both functions have
 * several return points and a counter that leaks is worse than no
 * counter at all. A counter rather than a flag because led_prompt()
 * can be entered from inside led_input().
 *
 * See sw/ext/nextvi/ZEITLOS.md.
 */
int vi_typing;
...
int led_input(sbuf *sb, char *post, int postn, int row, int flg, int *pren)
{
	int r;
	vi_typing++;
	r = led_input_inner(sb, post, postn, row, flg, pren);
	vi_typing--;
	return r;
}
...
int led_prompt(sbuf *sb, char *insert, int *kmap, ins_state *is, int ps, int flg)
{
	int r;
	vi_typing++;
	r = led_prompt_inner(sb, insert, kmap, is, ps, flg);
	vi_typing--;
	return r;
}
