...
Licence: the layout data is xkeyboard-config's, under its X11/MIT-style
notices. The generated file says so in its header and they are kept,
verbatim, in sw/common/zkbd_layouts.LICENSE -- update that file (from
the xkb-data package's copyright file) when re-syncing to a newer
xkeyboard-config. keysymdef.h is only read to turn keysym names into
Unicode codepoints; nothing of it is copied into the output.
"""
...
def main():
	...
	w("/*")
	w(" * The layout data below is derived from xkeyboard-config, the X keyboard")
	w(" * configuration database, and is covered by its copyright and permission")
	w(" * notices, reproduced in zkbd_layouts.LICENSE beside this file. The")
	w(" * dead-key combinations are computed from Unicode normalization.")
	w(" */")
	w("")
	w("#include <stdint.h>")
	w('#include "zkbd.h"')
